...
def schedule(string, step):
    try:
        ...
        if match:
            ...
            return (1 - mix) * initial + mix * final  # initial + mix(final-initial)
        ...
        if match:
            ...
            return (initial - final) * 0.5 ** (step / halflife) + final  # final - (final - initial) * 0.5 ** (step / halflife)
...
def lambda_return(reward, value, pcont, bootstrap, lambda_, axis):
    ...
    if axis != 0:
        reward = tf.transpose(reward, dims)  # （T x B, 15）
        value = tf.transpose(value, dims)  # （T x B, 15）
        pcont = tf.transpose(pcont, dims)  # （T x B, 15）

    if bootstrap is None:
        bootstrap = tf.zeros_like(value[-1])  
    
    
    next_values = tf.concat([value[1:], bootstrap[None]], 0)  # (15, T x B)
...
def static_scan(fn, inputs, start, reverse=False):
    ...
    # print(inputs[0].shape, inputs[0].shape, tf.nest.flatten(inputs)[0].shape[0], len(tf.nest.flatten(start)))
