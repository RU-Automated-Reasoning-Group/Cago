...
class GC_goal_picker:
    ...
    def make_get_goal_fn(self, config, agnt, sample_env_goals_fn):
        ...
        def get_goal(obs, state=None, mode='train', env=None, **kwargs):
            ...
            obs = {key: value for key, value in obs.items() if key not in [
                'env_states']}
            ...
            if config.goal_strategy == "Greedy":
                ...
                if config.if_image_obs:
                    goal, image_goal = self.goal_strategy.search_goal(
                        env=env, **kwargs)
    ...
    def sample_goal(self, obs, state=None, mode='train', **kwargs):
        ...
        if self.random:
            ...
            if self.if_exploration_potential_filter:
                ...
                # post: {''stoch': z_t+1, 'deter': h_t+1}
                post, prior = self.wm.rssm.observe(
                    embed, action, obs['is_first'], latent)
                # for k,v in post.items():

                init_start = {k: v[:, -1]
                              for k, v in post.items()}  # (1, 1, d) --> (1, d)
...
class Demo_goal_Planner:
    ...
    def search_goal(self, env, **kwargs):
        ...
        if self.demo_search_strategy == 0:
            ...
            truth, imagined_decoded, demo_prediction_error_list = self.Predict_demotra_by_wm(
                demo_trajectory, self.wm)

            # print("demo_prediction_error_list:", demo_prediction_error_list)

            demo_prediction_error_list_flat = tf.reshape(
                demo_prediction_error_list, [-1])

            demo_prediction_error_list_flat_ignore_first = demo_prediction_error_list_flat[
                1:]

            avg_value = tf.reduce_mean(
                demo_prediction_error_list_flat_ignore_first)

            index = tf.argmax(
                tf.cast(demo_prediction_error_list_flat_ignore_first > avg_value, tf.int32))
            ...
            self.demo_wm_error_metric = tf.reduce_mean(
                demo_prediction_error_list_flat[2:])

            demo_prediction_error_list_flat_ignore = demo_prediction_error_list_flat[
                start_index:]

            avg_value = tf.reduce_mean(demo_prediction_error_list_flat_ignore)

            index = tf.argmax(
                tf.cast(demo_prediction_error_list_flat_ignore > avg_value, tf.int32))
        ...
        # Multi-step imagine
            ...
            truth, imagined_decoded, demo_prediction_error_list = self.Predict_demotra_by_wm_2(
                demo_trajectory, self.wm)
        ...
        # ten-step imagine for each step
            ...
            truth, imagined_decoded, demo_prediction_error_list = self.Predict_demotra_by_wm_3(
                demo_trajectory, self.wm)
            ...
            median_value = tfp.stats.percentile(
                demo_prediction_error_list_flat_ignore, 50.0)

            index = tf.argmax(
                tf.cast(demo_prediction_error_list_flat_ignore > median_value, tf.int32))
            ...
            quarter_percentile_value = tfp.stats.percentile(
                demo_prediction_error_list_flat_ignore, 25.0)

            index = tf.argmax(tf.cast(
                demo_prediction_error_list_flat_ignore > quarter_percentile_value, tf.int32))
            ...
            values, indices = tf.math.top_k(
                demo_prediction_error_list_flat_ignore_first, k=(length-error_rank+1))
            ...
            start_index = max(
                int(length * min(self.learning_rate+0.2, 1)) - 1, 1)
            ...
            normalized_errors = demo_prediction_error_list_flat_ignore / \
                tf.reduce_sum(demo_prediction_error_list_flat_ignore)

            normalized_probs = normalized_errors.numpy()
            chosen_idx = np.random.choice(
                len(normalized_probs), p=normalized_probs)
            ...
            normalized_squared_errors = squared_errors / \
                tf.reduce_sum(squared_errors)
            ...
            index = tf.argmax(tf.cast(
                demo_prediction_error_list_flat_ignore_first > error_threshold, tf.int32))
            ...
            normalized_demo_prediction_error_list_flat_range = demo_prediction_error_list_flat_range / \
                tf.reduce_sum(demo_prediction_error_list_flat_range)
            ...
            quarter_percentile_value = tfp.stats.percentile(
                demo_prediction_error_list_flat_ignore_first, 75.0)
            ...
            if self.if_eval_peg:
                ...
                all_demo_tra, tra_num = self.get_all_demo_trajectory_by_env(
                    env)
                ...
                for i in range(tra_num):
                    ...
                    for key, value in demo_trajectory.items():
                        obs[key] = np.expand_dims(
                            np.expand_dims(value[0], axis=0), axis=0)
                    embed = self.wm.encoder(obs)
                    # posterior is q(s' | s,a,e)
                    # post: {''stoch': z_t+1, 'deter': h_t+1}
                    post, prior = self.wm.rssm.observe(
            ...
            index = tf.cond(
                ...
                lambda: tf.argmax(
                    demo_prediction_error_list_flat_ignore_first),
                lambda: tf.argmax(
                    tf.cast(demo_prediction_error_list_flat_ignore_first > threshold, tf.int32))
            )
            ...
            self.demo_wm_error_metric = tf.reduce_mean(
                eval_value_flat_ignore_first)

            # Square the values
            squared_value = tf.exp(eval_value_flat_ignore_first)

            normalized_squared_value = squared_value / \
                tf.reduce_sum(squared_value)
            ...
            normalized_demo_prediction_error_list_flat_ignore_first = demo_prediction_error_list_flat_ignore_first / \
                tf.reduce_sum(demo_prediction_error_list_flat_ignore_first)
            ...
            threshold = np.clip(demo_obs_count_median,
                                self.config.count_lb, self.config.count_ub)
            ...
            min_index = max(0, index - max(len(demo_obs) //
                            self.config.sample_range, 1))
            max_index = min(len(demo_obs), index +
                            max(len(demo_obs) // self.config.sample_range, 1))
    ...
    def Predict_demotra_by_wm(self, demo_trajectory, wm, use_post_states=False):
        ...
        demo_tra = {key: tf.identity(value)
                    for key, value in demo_trajectory.items()}
        ...
        post_states, prior_states = wm.rssm.observe(
            embed[:, :], demo_tra['action'][:, :], demo_tra['is_first'][:, :])  # one step imagine

        if use_post_states:
            imagined_decoded = tf.cast(decoder(wm.rssm.get_feat(post_states))[
                                       self.config.state_key].mode()[:], tf.float32)
        else:
            imagined_decoded = tf.cast(decoder(wm.rssm.get_feat(prior_states))[
                                       self.config.state_key].mode()[:], tf.float32)

        # tf.print("Imagined Decoded:", imagined_decoded, summarize=-1)

        square_error_list.append(tf.reduce_sum(
            tf.square(truth - imagined_decoded), axis=2))
    ...
    def Predict_demotra_by_wm_2(self, demo_trajectory, wm):
        ...
        post_states, prior_states = wm.rssm.observe(
            embed[:, :2], demo_tra['action'][:, :2], demo_tra['is_first'][:, :2])
        init = {k: v[:, -1] for k, v in post_states.items()}
        prior = wm.rssm.imagine(demo_tra['action'][:, 2:], init)
        totally_imagined_decoded = tf.cast(decoder(wm.rssm.get_feat(prior))[
                                           self.config.state_key].mode()[:], tf.float32)

        square_error_list.append(tf.reduce_sum(
            tf.square(truth[:, 2:, :] - totally_imagined_decoded), axis=2))
    ...
    def Predict_demotra_by_wm_3(self, demo_trajectory, wm):
        ...
        for key, value in demo_tra.items():
            # Expand tensor along the batch dimension
            demo_tra[key] = value[None, :]

        # Convert truth to float32
        truth = tf.cast(demo_tra[self.config.state_key], tf.float32)
        embed = wm.encoder(demo_tra)
        ...
        for t in range(2, total_steps):
            # Adjust if fewer than 10 steps are left
            remaining_steps = min(10, total_steps - t)

            # Observe the current step to update init
            post_states, _ = wm.rssm.observe(
                embed[:, t:t+1], demo_tra['action'][:, t:t+1], demo_tra['is_first'][:, t:t+1])
            # Update init for the current step
            init = {k: v[:, -1] for k, v in post_states.items()}

            # Imagine the next few steps starting from the updated init
            # Imagine the next steps
            prior = wm.rssm.imagine(
                demo_tra['action'][:, t:t+remaining_steps], init)
            imagined_decoded = tf.cast(decoder(wm.rssm.get_feat(prior))[
                                       self.config.state_key].mode()[:], tf.float32)

            # Calculate the squared error for the imagined steps
            error = tf.reduce_mean(
                tf.square(truth[:, t:t+remaining_steps, :] - imagined_decoded), axis=[1, 2])
    ...
    def Eval_demotra_by_dd(self, demo_trajectory, wm):
        ...
        post_states, prior_states = wm.rssm.observe(
            embed[:, :], demo_tra['action'][:, :], demo_tra['is_first'][:, :])
        ...
        dd_pred = self.agnt._task_behavior.dynamical_distance(
            tf.concat([inp_embed, goal_embed], axis=-1))  # shape: (num_segments, 2*D)
    ...
    def sample_demo_trajectory_by_env_seed(self, env):
        ...
        if self.config.if_image_obs:
            ...
            target_key = ['observation', 'action',
                          'reward', 'is_last', 'is_first', 'images']

        else:
            target_key = ['observation', 'action',
                          'reward', 'is_last', 'is_first']
