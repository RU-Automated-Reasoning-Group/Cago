...
class EnsembleRSSM(common.Module):
    ...
    def __init__(
            ...
            act='elu', norm='none', std_act='softplus', min_std=0.1, max_std=2.0):
        super().__init__()
    ...
    def _suff_stats_layer(self, name, x):
        if self._discrete:
            ...
            std = tf.minimum(std, self._max_std)
...
class DistLayer(common.Module):
    ...
    def __init__(
            self, shape, dist='mse', min_std=0.1, max_std = 2.0, init_std=0.0):
        self._shape = shape
        self._dist = dist
        self._min_std = min_std
        self._max_std = max_std
...
class Cluster(nn.Module):
    ...
    def __init__(self, config, embed_size):
        ...
        # weight of every component of GMM
        self.mix_logit = nn.Parameter(torch.ones(self.n_mix), requires_grad=config.cluster["learned_prior"])
    ...
    def component_log_prob(self, x):
        ...
        if x.ndim == 1:
            ...
            x = x.unsqueeze(1).repeat(1, self.n_mix, 1)  # （B，D_x） --> (B, 1, D_x) --> (B, self.n_mix, D_x)
        ...
        comp_logstd = torch.clamp(self.comp_logstd, self.config.cluster["LOG_STD_MIN"], self.config.cluster["LOG_STD_MAX"])  
    ...
    def sample(self, candidate_num, sample_num):
        ...
        # print(log_comp_probs.shape)

        log_mix_probs = torch.log_softmax(self.mix_logit, dim=-1).unsqueeze(0)  
        log_prob_points_sampled  = torch.logsumexp(log_mix_probs + log_comp_probs, dim=-1, keepdim=True)  

        # print(log_prob_points_sampled.shape)

        log_prob_points_sampled = log_prob_points_sampled.squeeze()

        # print(log_prob_points_sampled.shape)

        # print(log_prob_points_sampled)

        
        min_prob_indices = torch.topk(log_prob_points_sampled, sample_num, largest=False).indices

        # print(log_prob_points_sampled[min_prob_indices])
    ...
    def forward(self, x, with_elbo=True):
        ...
        log_prob_x = torch.logsumexp(log_mix_probs + log_comp_probs, dim=-1, keepdim=True)  
    ...
    def assign_centroids(self, x):
        ...
        # weight of every component of GMM
        self.mix_logit = nn.Parameter(torch.ones(self.n_mix), requires_grad=self.config.cluster["learned_prior"])


class MLP_Classifier(common.Module):
    ...
    def __init__(self, shape, layers, units, act='elu', norm='none', **out):
        ...
        self._act = get_act(act)
        self._out = out
        self.output_layer = tfkl.Dense(1, activation='sigmoid')
