...
class Replay:
    ...
    def dataset_specific_label(self, batch, length, label):
        example = next(iter(self._generate_chunks_specific_label(length, label)))
        dataset = tf.data.Dataset.from_generator(
    ...
    def recent_dataset_specific_label(self, batch, length, label):
        example = next(iter(self._generate_recent_chunks_specific_label(length, label)))
        dataset = tf.data.Dataset.from_generator(
    ...
    def _sample_sequence(self):
        ...
        invalid_key  = ['env_states']
        sequence = {k: v for k, v in sequence.items() if k not in invalid_key}
        return sequence
